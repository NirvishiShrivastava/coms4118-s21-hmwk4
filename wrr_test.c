...
int main(int argc, char *argv[])
{
	...
	/*printf("Calling wrr_set system call\n");
	ret = wrr_set(1, buf, counter);

	if (ret < 0) {
		fprintf(stderr, "error from get call: %s\n", strerror(errno));
		printf("Get System Call Failed.\n");
		exit(EXIT_FAILURE);
        }
	printf("Retuned from GET call with counter value %ld\n", *counter);
